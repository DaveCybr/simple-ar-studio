...
-- ✅ FIXED VERSION - Production ready, no syntax errors
...
-- ========================================
-- STEP 3: Convert marker_data to JSONB
-- ========================================
DO $$ 
BEGIN
  ...
  IF EXISTS (
    ...
    AND column_name = 'marker_data' 
    AND data_type = 'text'
  ) THEN
    -- Convert TEXT to JSONB
    ALTER TABLE ar_content 
    ALTER COLUMN marker_data TYPE JSONB USING 
      CASE 
        WHEN marker_data IS NULL THEN NULL
        WHEN marker_data = '' THEN NULL
        ELSE marker_data::jsonb
      END;
...
-- ========================================
-- STEP 7: Drop old functions and triggers
-- ========================================
...
-- ========================================
-- STEP 9: Create marker validation function
-- ========================================
...
BEGIN
  ...
  IF NEW.library = 'arjs' THEN
    ...
    IF NEW.marker_data->>'markerType' NOT IN ('pattern', 'barcode', 'hiro', 'kanji') THEN
      RAISE EXCEPTION 'Invalid markerType for AR.js';
    END IF;
  END IF;
...
-- ========================================
-- STEP 10: Create trial validation function
-- ========================================
...
BEGIN
  ...
  -- Return true if trial is active and not expired
  RETURN (
...
-- ========================================
-- STEP 11: Fix all quota mismatches BEFORE adding constraints
-- ========================================
DO $$
BEGIN
  -- Fix demo users with wrong quota
  UPDATE profiles 
  SET upload_quota = 3
  WHERE subscription_tier = 'demo' AND upload_quota != 3;

  -- Fix pro users with wrong quota
  UPDATE profiles 
  SET upload_quota = 20
  WHERE subscription_tier = 'pro' AND upload_quota != 20;

  -- Fix pro_plus users with wrong quota
  UPDATE profiles 
  SET upload_quota = 30
  WHERE subscription_tier = 'pro_plus' AND upload_quota != 30;

  -- Fix uploads_used that exceed quota
  UPDATE profiles
  SET uploads_used = upload_quota
  WHERE uploads_used > upload_quota;

  RAISE NOTICE '✅ Fixed all quota mismatches';
END $$;

-- ========================================
-- STEP 12: Drop old constraints
-- ========================================
...
-- ========================================
-- STEP 13: Add new constraints (now safe)
-- ========================================
...
CHECK (
  (subscription_tier = 'demo' AND upload_quota = 3) OR
